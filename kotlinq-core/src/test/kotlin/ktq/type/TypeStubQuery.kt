...
class TypeStubQueryable {
  ...
  @Test fun `no arg type stub is possible`() {
    val query = object : QModel<Order>(Order) {
      val personWhoPlacedTheOrder by model.owner.query(TypeStubQueryable::Me)
    }
  ...
  @Test fun `noArg type stub multiple custom arguments toGraphQL()`() {
    val query = object : QModel<Order>(Order) {
      val personWhoPlacedTheOrder by model.owner.query(TypeStubQueryable::Me).invoke {
        config {
          "Hello" with "World"
          "Foo" with 1337
          "Bar" with false
  ...
  @Test fun `optionalArg type stub with no arguments is possible`() {
    val query = object : QModel<Order>(Order) {
      val address by model.ownerAddress.query(TypeStubQueryable::DefaultAddress)
    }
    assertThat(query.toGraphql())
  ...
  @Test fun `optionalArg type stub with arguments is possible`() {
    val query = object : QModel<Order>(Order) {
      val address by model.ownerAddress.query(TypeStubQueryable::DefaultAddress)(Order.OwnerAddressArgs()) {
        config {
  ...
  @Test fun `required config type stub enforces argument passed to stub`() {
    val query = object : QModel<Order>(Order) {
      val destinationAddress by model.destination.query(TypeStubQueryable::DefaultAddress)(
